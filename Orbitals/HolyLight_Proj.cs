...
namespace Virtuous.Orbitals
{
    public class HolyLight_Proj : ModProjectile
    {
        ...
        private const float OrbSpeed = 1/30f * Tools.RevolutionPerSecond; //Orbiting speed
        ...
        public override void AI()
        {
            ...
            if (!orbitalPlayer.active[OrbitalID.HolyLight]) //Keep it alive only while the summon is active
            {
                projectile.Kill();
            }
            else
            {
                ...
                relativePos = relativePos.RotatedBy(OrbSpeed).SafeNormalize(Vector2.UnitX) * distance; //Rotates the projectile around the player and resets its distance
