...
namespace Virtuous.Orbitals
{
    public class SpiralSword_Proj : ModProjectile
    {
        ...
        //Main
        private const float AngularSpeed = 1/2f * Tools.RevolutionPerSecond; //How fast it orbits around the player
        ...
        public override void AI()
        {
            ...
            if (!orbitalPlayer.active[OrbitalID.SpiralSword]) //Keep it alive only while the summon is active
            {
                projectile.Kill();
            }
            else
            {
                ...
                if (orbitalPlayer.time > DyingTime)
                {
                    ...
                    relativePos = relativePos.RotatedBy(newAngSpeed).SafeNormalize(Vector2.UnitX) * distance; //Rotates the sword around the player and resets its distance
                ...
                else //timeLeft actually starts going down from dyingTime while it flies away
                {
                    ...
                    projectile.Center += relativePos.SafeNormalize(Vector2.UnitY) * shootSpeed; //Projectile moves forward in the direction of its rotation
        ...
        {
            return new Color(28, 255, 255, 150) * projectile.Opacity;
        }
    }
}
