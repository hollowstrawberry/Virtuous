...
namespace Virtuous.Orbitals
{
    public class Shuriken_Proj : ModProjectile
    {
        ...
        private const float OrbSpeed = 1 * Tools.RevolutionPerSecond; //Orbiting speed
        ...
        private const float DyingOrbSpeed = 4 * Tools.RevolutionPerSecond; //Final spin speed
        ...
        public override void AI()
        {
            ...
            if (!orbitalPlayer.active[OrbitalID.Shuriken]) //Keep it alive only while the summon is active
            {
                projectile.Kill();
            }
            else
            {
                ...
                relativePos = relativePos.RotatedBy(newOrbitingSpeed).SafeNormalize(Vector2.UnitX) * distance; //Rotates the shuriken around the player and resets its distance
